...
Зачем: раньше все 41 признак были прописаны прямо в коде расчёта.
Добавить свой означало править ядро программы, а убрать ненужные —
никак. Теперь признаки собраны в именованные наборы, а исследователь
может дописать собственный, не трогая существующий код.

Это то, о чём говорил научный руководитель: «можно конечно сделать
все эти списки, которые признаки вычисляют, тоже открытыми, то есть
клиент может потом добавить чего-то».
