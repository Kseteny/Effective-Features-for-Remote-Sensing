...
def forward_selection_mi(dataset, mask, cfg: ExperimentConfig,
    ...
    Похожесть меряется корреляцией. 
    Возвращает (selected_indices, history_values).
    """
    c = dataset.shape[-1]
    flat = mask.flatten()
    ...
    # ту же, что используется для kNN, чтобы критерии смотрели на сопоставимые данные.
    ...
    # Оба слагаемых считаются один раз по всем признакам, дальше отбор только берёт из них нужные значения - иначе метод был бы медленным.
    ...
    # Релевантность приводим к диапазону 0…1: она измеряется в натах, а избыточность - в долях единицы, и вычитать их напрямую
