...
Зачем: раньше результаты жили только в памяти процесса. Стоило перезапустить
сервер — и всё пропадало. Теперь законченные расчёты складываются в SQLite,
и к ним можно вернуться хоть через неделю.
